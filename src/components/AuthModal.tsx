  ...
  const handleEmailLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (newAttempts >= 5) {
        ...
        const msg = err.message || '';
        if (msg.includes('Wrong email or password') || msg.includes('Invalid login credentials')) {
          setError(`Wrong email or password. ${5 - newAttempts} attempt${5 - newAttempts > 1 ? 's' : ''} remaining.`);
        } else {
          setError(msg || 'Something went wrong. Please try again.');
        }
      }
